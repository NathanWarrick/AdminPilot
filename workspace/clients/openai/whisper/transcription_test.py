...
result = model.transcribe(r'workspace\clients\openai\whisper\test_files\transcribing_1.mp3')
...
output = query({
	"inputs": "Summarise the following transription in a form that would be suitable for a councils minutes, complete with enough detail that somoene that didn't attend the meeting would understand everything that happened:" + response,
})
print(output)