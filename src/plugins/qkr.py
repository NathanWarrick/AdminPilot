...
class Browser_Headless:  # Selenium Browser Configuration
    ...
    def page_height(self):
        S = lambda X: self.browser.execute_script(
            "return document.body.parentNode.scroll" + X
        )
        self.browser.set_window_size(
            S("Width"), S("Height")
        )  # May need manual adjustment
        self.browser.find_element(by=By.TAG_NAME, value="body").screenshot(
            "web_screenshot.png"
        )
...
def main():
    ...
    for view_button_pos in pyautogui.locateAllOnScreen(
        r"workspace\bots\qkr_bot\assets\view_list.png"
    ):  # Create an array of values for the Y location of the QKR view list buttons so i can click them later with pyautogui
    ...
    for x in view_button_array:  # All code needs to go in this loop
        ...
        x, y = pyautogui.locateCenterOnScreen(
            r"workspace\bots\qkr_bot\assets\download_exit.png",
            region=(0, 700, 1920, 1080),
        )
        ...
        paths = [
            os.path.join(r"workspace\bots\qkr_bot\Downloads", basename)
        ...
        excursionname = name[
            32 : name.find("-")
        ]  # Trim string to just the name of the form
        ...
        if excursionname == "\LocalExcursionForm":
            ...
            qkr_df[["First Name", "Last Name"]] = (
                qkr_df["Student Name:"]
                .loc[qkr_df["Student Name:"].str.split().str.len() == 2]
            ...
            qkr_df = qkr_df.rename(
                columns={
                    "Parent/Carer's Name:": "Guardian's Name",
            ...
            qkr_df[["First Name", "Last Name"]] = (
                qkr_df["Students Full Name:"]
                .loc[qkr_df["Students Full Name:"].str.split().str.len() == 2]
                .str.split(expand=True)
            )  # Split students name into first and last
            ...
            try:
                qkr_df = qkr_df[
                    [
                        ...
                        "Parent/Carer's business hours number:",
                        "Swimming Ability:",
                        "Tick all relevant conditions",
                        "If other, include any other diagnosed physical or mental health conditions",
                        "Please tick if you are allergic to any of the following",
                        "If other please list all other allergies",
                        "What special care is recommended for these allergies?",
                        "Is your child taking any medicine(s)?",
                ...
                qkr_df = qkr_df.rename(
                    columns={
                        "Parent/Carer's Full Name:": "Guardian's Name",
                        "Parent/Carer's business hours number:": "Contact Number",
